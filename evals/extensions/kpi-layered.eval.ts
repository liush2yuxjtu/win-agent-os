...
export default defineEval({
  ...
  async test(t) {
    await t.send("无论当前布局是否已是基础款，都直接读取基础款 spec 并调用 render_ui 预览，不要追问");
    ...
    t.calledTool("render_ui", {
      input: {
        spec: (value) => {
          ...
          const layeredKpi = /k0Value/.test(spec) && /"type":\s*"Heading"/.test(spec) && /k0Delta/.test(spec) && /"type":\s*"Badge"/.test(spec);
          const noFlatTitle = !/"title":\s*\{\s*"\$template":\s*"\$\{?\/kpis\/0\/value/.test(spec);
          const contractQueryId = /fixed:anchor|fixed:daily|fixed:topMaterials|fixed:insights/.test(spec);
