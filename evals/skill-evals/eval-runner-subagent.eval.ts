...
export default defineEval({
  ...
  async test(t) {
    await t.send(
      "【验收测试·必须调用工具】用 eval-runner 子代理执行 skill-creator 的一个纯文本功能评估用例" +
        "(用例 prompt:根据技能指令列出 SKILL.md 必须包含的两个 frontmatter 字段,不要调用任何工具)," +
        "然后把子代理返回的判定结果(verdict/evidence)汇报给我。\n" +
        "硬性要求:" +
        "这是自动化验收测试,你的工具调用序列会被程序化检查,序列必须是:load_skill → eval-runner → (汇报)。" +
        "① 第一步:调用 load_skill 工具加载 skill-creator(沙盒 read_file 看不到宿主技能包,load_skill 是宿主技能唯一入口)," +
