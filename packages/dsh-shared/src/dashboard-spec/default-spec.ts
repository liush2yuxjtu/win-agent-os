 ...
 *  - 走势图/洞察/明细通过元素 props.dataRef 引用 Query Registry（FIXED_QUERY_IDS 契约常量，
 *    单一来源，业务包间不硬编码 queryId），渲染时由 data-binding 层拉取注入。
 */

import { FIXED_QUERY_IDS } from "../qc-dashboard/queries.ts";
  ...
  return {
    ...
    elements: {
      ...
      chart: {
        type: "BarChart",
        props: {
          dataRef: { queryId: FIXED_QUERY_IDS.daily, field: "rows" },
          xKey: "stat_date",
      ...
      insightsTitle: {
        type: "Heading",
        props: { dataRef: { queryId: FIXED_QUERY_IDS.insights, field: "title" }, level: "h3" },
        children: [],
      },
      insightsDesc: {
        type: "Text",
        props: { dataRef: { queryId: FIXED_QUERY_IDS.insights, field: "description" }, variant: "muted" },
        children: [],
      },
      topTable: {
        type: "Card",
      ...
      topTableInner: {
        type: "Table",
        props: { dataRef: { queryId: FIXED_QUERY_IDS.topMaterials, field: "rows" } },
        children: [],
      },
      qualityRow: {
        type: "Stack",
