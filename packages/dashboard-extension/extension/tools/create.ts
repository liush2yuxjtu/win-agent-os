...
export default defineTool({
	...
	inputSchema: z.object({
		type: z
			.string()
			.describe(
				"新元素类型：Card / Table / BarChart / Heading / Separator / Badge / Alert / Text / Stack / Grid",
			),
		props: z
		...
		key: z
			...
			.optional()
			.describe(
				"可选：卡片在 spec.elements 里的 key（缺省自动生成 card/card1/…）",
			),
	}),
	async execute({ type, props, key }) {
		...
		return {
			ok: true,
			spec: result.spec,
			hint: "调用 render_ui 渲染此 spec 供用户预览确认",
		};
	},
});
